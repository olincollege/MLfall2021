...
\todo[inline]{Redo this motivation, as they have not seen Titanic and we didn't give them the walk-through in the same way}
...
In order to motivate the idea of neural networks, we'll be examining the Titanic Kaggle dataset. 
\todo{oh no, I've messed up the careful flow of the class by switching datasets in the last homework!}
...
\todo[inline]{go through and fix all the titanic references to age and sex unless we also introduce them to the dataset}
...
\item Please add your own links on the Discord chat.
\ei
\end{externalresources}
